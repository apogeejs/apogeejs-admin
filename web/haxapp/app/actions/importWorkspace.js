...
haxapp.app.importworkspace.getImportCallback = function(app,parentGenerator) {
    return function() {
        ...
        var onOpen = function(err,workspaceData,workspaceHandle) {
            ...
            if(err) {
                ...
                //open workspace
                haxapp.app.importworkspace.openWorkspace(app,parentGenerator,workspaceData,workspaceHandle,actionCompletedCallback);
            }
        }    
...
haxapp.app.importworkspace.openWorkspace = function(app,parentGenerator,workspaceText,workspaceHandle,actionCompletedCallback) {
...
//I should verify the file type and format!    
		...
		var workspaceImportDialogFunction = haxapp.app.addcomponent.getAddComponentCallback(app,parentGenerator,newParentOptionsJson,newChildComponentsJson);
...
haxapp.app.importworkspace.openWorkspaceFromUrl = function(app,url) {
    var actionCompletedCallback = function(actionResponse) {
        if(!actionResponse.getSuccess()) {
            alert(actionResponse.getErrorMsg());
        }
    ...
    haxapp.app.importworkspace.openWorkspaceFromUrlImpl(app,url,actionCompletedCallback);
}

/** This method opens an workspace by getting the workspace file from the url. */
haxapp.app.importworkspace.openWorkspaceFromUrlImpl = function(app,url,actionCompletedCallback) {
    var onDownload = function(workspaceText) {
        haxapp.app.importworkspace.openWorkspace(app,workspaceText,url,actionCompletedCallback);
    }
    ...
    haxapp.app.importworkspace.doRequest(url,onDownload,onFailure);   
}

/**
 * This is an http request for the worksheet data
 */
haxapp.app.importworkspace.doRequest= function(url,onDownload,onFailure) {
