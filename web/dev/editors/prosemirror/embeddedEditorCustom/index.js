...
//    this.updating = false
  ...
  valueChanged() {
      ...
      //THIS NEW THING DIDN'T WORK
    let newText = this.cm.getValue();
    if(this.node.textContent != newText) {
        let start = this.getPos() + 1;
        let end = start + this.node.textContent.length;
        let tr = this.view.state.tr.replaceWith(start,end,schema.text(newText));
        this.view.dispatch(tr);
    } 
    
    //THIS WAS THIS ORIGINAL THING
